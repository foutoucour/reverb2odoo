# Gear / Listing Split — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Split `x_guitar` (which conflates a physical instrument with a sale listing) into `x_gear` (the physical object you own) and `x_listing` (one record per platform per time it's listed for sale), enabling multi-platform listings, full sale history, and richer physical details.
...
> **Critical naming rule for implementors:** `conn.get_model(name)` for **data operations** (search_read, create, write) always takes the **underscored** form: `"x_gear"`, `"x_listing"`, `"x_guitar"`. The **dotted** form (`"x.gear"`, `"x.guitar"`) is used only when querying `ir.model` or `ir.model.fields` to inspect the schema. Getting this wrong causes a silent `False` result on searches with no error message.
...
x_models          (unchanged — model catalog: brand, specs, reverb category)
  └─ x_gear (new — the physical object: serial, condition, acquisition, keeper status)
       └─ x_listing (new — one per platform per listing event: URL, price, availability)
x_expense         (move many2one link from x_guitar → x_gear)
```
...
| `x_models_id`             | Model                | many2one → x_models | replaces x_guitar.x_studio_models    |
...
| `x_custom_build_part_ids`      | Custom Parts         | many2many → x_custom_build_part | migrated from x_guitar.x_studio_parts |
...
| `x_gear_id`        | Instrument           | many2one → x_gear | core FK                            |
...
| `x_is_taxed`                | Taxed                | boolean   | migrated from x_guitar.x_studio_taxed          |
| `x_can_accept_offers`        | Accept Offers        | boolean   | migrated from x_guitar.x_studio_accept_offers  |
...
| `migrations/002_create_x_gear.py` | **Create** | Create x_gear model + all fields via ir.model / ir.model.fields |
| `migrations/003_create_x_listing.py` | **Create** | Create x_listing model + all fields |
| `migrations/004_migrate_data.py` | **Create** | Read x_guitar → write x_gear + x_listing (1:1 at migration time) |
| `migrations/005_link_expenses.py` | **Create** | Update x_expense.x_studio_guitar many2one to point at x_gear |
...
| `tests/migrations/test_002_x_gear.py` | **Create** | Verify x_gear model + fields exist with correct types |
...
| `sync_model.py` | **Modify** | Write to x_listing + x_gear instead of x_guitar |
...
## Task 2 — Create `x_gear`

**Files:**
- Create: `migrations/002_create_x_gear.py`
- Create: `tests/migrations/conftest.py`
- Create: `tests/migrations/test_002_x_gear.py`
...
```python
# tests/migrations/test_002_x_gear.py
"""Verify x_gear model and fields exist after migration 002."""
from __future__ import annotations
...
EXPECTED_FIELDS = [
    ...
    ("x_custom_build_part_ids", "many2many"),
...
def test_x_gear_field_exists(odoo_conn, field_name: str, expected_ttype: str) -> None:
    ir_fields = odoo_conn.get_model("ir.model.fields")
    results = ir_fields.search_read(
        [("model", "=", "x.gear"), ("name", "=", field_name)],
        ["ttype"],
    )
    assert results, f"Field x_gear.{field_name} does not exist"
    assert results[0]["ttype"] == expected_ttype, (
        f"x_gear.{field_name}: expected {expected_ttype!r}, "
        f"got {results[0]['ttype']!r}"
    )


def test_x_gear_model_exists(odoo_conn) -> None:
    ir_model = odoo_conn.get_model("ir.model")
    results = ir_model.search_read([("model", "=", "x.gear")], ["id"])
    assert results, "x_gear model does not exist in Odoo"
```

- [ ] **Step 2: Run the test to verify it fails**

```bash
uv run pytest tests/migrations/test_002_x_gear.py -v 2>&1
```

Expected: All tests FAIL with "x_gear model does not exist"

- [ ] **Step 3: Write the migration script**

```python
# migrations/002_create_x_gear.py
"""Create the x_gear custom model in Odoo Studio."""
from __future__ import annotations
import json
import os
import sys
...
def run(conn) -> None:
    mid = ensure_model(conn, "x.gear", "Gear")

    # Core identity
    ensure_field(conn, "x.gear", mid, "x_name", "Description", "char",
                 required=True)
    ensure_field(conn, "x.gear", mid, "x_models_id", "Model", "many2one",
                 relation="x.models")
    ensure_field(conn, "x.gear", mid, "x_serial_number", "Serial Number", "char")
    ensure_field(conn, "x.gear", mid, "x_year", "Year", "integer")
    ensure_field(conn, "x.gear", mid, "x_condition", "Condition", "selection",
    ...
    # Physical dimensions
    ensure_field(conn, "x.gear", mid, "x_weight_lbs", "Weight (lbs)", "float")
    ensure_field(conn, "x.gear", mid, "x_nut_width_mm", "Nut Width (mm)", "float")
    ensure_field(conn, "x.gear", mid, "x_neck_profile", "Neck Profile", "selection",
                 selection=json.dumps([
                     ["C", "C"], ["U", "U"], ["D", "D"], ["V", "V"],
                     ["asymmetric", "Asymmetric"],
                 ]))
    ensure_field(conn, "x.gear", mid, "x_fretboard_radius_in",
                 "Fretboard Radius (in)", "float")
    ensure_field(conn, "x.gear", mid, "x_fret_count", "Fret Count", "integer")
    ensure_field(conn, "x.gear", mid, "x_fret_size", "Fret Size", "selection",
                 selection=json.dumps([
                     ["small", "Small"], ["medium", "Medium"],
                     ["medium_jumbo", "Medium Jumbo"],
                     ["jumbo", "Jumbo"], ["wide_fat", "Wide/Fat"],
                 ]))
    ensure_field(conn, "x.gear", mid, "x_modifications", "Modifications", "text")

    # Custom build
    ensure_field(conn, "x.gear", mid, "x_is_custom_build", "Custom Build", "boolean")
    ensure_field(conn, "x.gear", mid, "x_custom_build_part_ids", "Custom Parts",
                 "many2many", relation="x.custom_build_part")

    # Acquisition
    ensure_field(conn, "x.gear", mid, "x_acquisition_date", "Acquired On", "date")
    ensure_field(conn, "x.gear", mid, "x_acquisition_price",
                 "Acquisition Price", "float")
    ensure_field(conn, "x.gear", mid, "x_acquisition_source",
    ...
    # Financial
    ensure_field(conn, "x.gear", mid, "x_currency_id", "Currency", "many2one",
                 relation="res.currency")
    ensure_field(conn, "x.gear", mid, "x_notes", "Notes", "text")

    # Migration traceability — link back to source x_guitar record
    ensure_field(conn, "x.gear", mid, "x_guitar_id", "Source Guitar (archive)",
                 "many2one", relation="x.guitar")

    logger.success("x_gear model ready")
...
```bash
uv run python migrations/002_create_x_gear.py 2>&1
```

Expected: `x_gear model ready`
...
```bash
git add migrations/002_create_x_gear.py \
        tests/migrations/conftest.py \
        tests/migrations/test_002_x_gear.py
git commit -m "feat: create x_gear Odoo model with physical and acquisition fields"
```
...
def run(conn) -> None:
    ...
    ensure_field(conn, "x.listing", mid, "x_name", "Description", "char",
                 required=True)
    ensure_field(conn, "x.listing", mid, "x_gear_id", "Gear", "many2one",
                 relation="x.gear", required=True)
    ...
    ensure_field(conn, "x.listing", mid, "x_is_taxed", "Taxed", "boolean")
    ensure_field(conn, "x.listing", mid, "x_can_accept_offers", "Accept Offers", "boolean")
    ...
    ensure_field(conn, "x.listing", mid, "x_currency_id", "Currency", "many2one",
                 relation="res.currency")
    ensure_field(conn, "x.listing", mid, "x_tax_id", "Tax Rate", "many2one",
                 relation="x.tax")
...
## Task 4 — Migrate data: x_guitar → x_gear + x_listing

**Files:**
- Create: `migrations/004_migrate_data.py`
- Create: `tests/migrations/test_004_data.py`

> **IMPORTANT:** Run `000_snapshot.py` before this step if you haven't already. This migration is idempotent (it checks `x_guitar_id` on x_gear before creating). Still: take a backup if your Odoo plan allows it.

- [ ] **Step 1: Write the failing data test (pre-migration state)**

```python
# tests/migrations/test_004_data.py
"""Verify data integrity after migration 004: x_guitar → x_gear + x_listing."""
from __future__ import annotations
import json
from pathlib import Path
import pytest
...
def test_instrument_count_matches_guitar_count(odoo_conn, snapshot) -> None:
    """Every x_guitar must have exactly one x_gear after migration."""
    expected = snapshot["x_guitar_total"]
    actual = odoo_conn.get_model("x_gear").search_count([])
    assert actual == expected, (
        f"x_gear count ({actual}) != x_guitar count at snapshot ({expected})"
    )
...
def test_all_listings_linked_to_instrument(odoo_conn) -> None:
    """No x_listing should have a null x_gear_id."""
    unlinked = odoo_conn.get_model("x_listing").search_count(
        [("x_gear_id", "=", False)]
    )
    assert unlinked == 0, f"{unlinked} listings have no x_gear_id"


def test_all_instruments_linked_to_model(odoo_conn) -> None:
    """No x_gear should have a null x_models_id."""
    unlinked = odoo_conn.get_model("x_gear").search_count(
        [("x_models_id", "=", False)]
    )
    assert unlinked == 0, f"{unlinked} instruments have no x_models_id"
...
"""
Migrate every x_guitar record to one x_gear + one x_listing.

Idempotent: skips guitars that already have an x_gear linked
via x_guitar_id.
"""
from __future__ import annotations
import os
...
def run(conn) -> None:
    ...
    instrument_model = conn.get_model("x_gear")
    ...
    while True:
        ...
        for g in guitars:
            ...
            if model_id_val:
                instrument_vals["x_models_id"] = model_id_val
            # NOTE: if x_studio_models is unset, x_models_id will be null.
            ...
            if g.get("x_studio_parts"):
                instrument_vals["x_custom_build_part_ids"] = [[6, 0, g["x_studio_parts"]]]
            ...
            for src, dst in [
                ...
                ("x_studio_taxed",           "x_is_taxed"),
                ("x_studio_accept_offers",   "x_can_accept_offers"),
            ...
            if g.get("x_studio_final_tax_rate_id"):
                listing_vals["x_tax_id"] = _resolve_id(g["x_studio_final_tax_rate_id"])
            if g.get("x_studio_currency_id"):
    ...
    logger.success(
        "Migration done: {} x_gear + {} x_listing created",
...
Expected: `Migration done: N x_gear + N x_listing created`
...
git commit -m "feat: migrate x_guitar data to x_gear and x_listing (1:1)"
```
...
`x_expense.x_studio_guitar` is a many2one pointing at `x_guitar`. After the migration each `x_guitar` has exactly one `x_gear`. Update expenses to point at the instrument.
...
then fills it using the x_guitar_id traceability field on x_gear.
"""
from __future__ import annotations
import os
import sys
...
def run(conn) -> None:
    ...
    ensure_field(
        ...
        "x_studio_instrument_id", "Gear", "many2one",
        relation="x.gear",
    )

    # 2. For each expense that has x_studio_guitar set, find the matching
    #    x_gear (via x_guitar_id) and fill x_studio_instrument_id.
    expense = conn.get_model("x_expense")
    instrument = conn.get_model("x_gear")
...
git commit -m "feat: relink x_expense from x_guitar to x_gear"
```
...
3. Create an `x_gear` if the listing is new (1 instrument per new listing — the user will manually consolidate duplicates later)
...
def _fetch_listings(conn, model_id: int) -> list[dict]:
    ...
    return listing.search_read(
        [
            ("x_gear_id.x_models_id", "=", model_id),
            ("x_platform", "=", "reverb"),
        ],
        LISTING_FIELDS,
    )
...
> **get_model() reminder:** All `conn.get_model()` calls in `sync_model.py` must use the underscored form: `"x_listing"`, `"x_gear"`, `"x_models"`. Never the dotted form — that is only for `ir.model` schema queries.

- [ ] **Step 4: Replace the create-new-entry logic**

When a Reverb listing URL is not found in x_listing, the sync currently creates one `x_guitar`. Replace with: create one `x_gear` + one `x_listing`:
...
def _create_instrument_and_listing(
    ...
    """Create an x_gear + x_listing for a new Reverb listing."""
    instrument_model = conn.get_model("x_gear")
    listing_model = conn.get_model("x_listing")
...
git commit -m "feat: sync_model writes to x_gear + x_listing instead of x_guitar"
```
...
instr = conn.get_model('x_gear').search_count([])
...
print(f'x_gear: {instr}')
print(f'x_listing: {lst}')
" 2>&1
```

Expected: `x_gear` and `x_listing` counts both equal the `x_guitar` count from the snapshot.
...
2. The `x_gear` and `x_listing` models can be archived in Odoo Studio without data loss.
3. The `x_guitar_id` field on `x_gear` and `x_listing` provides a full traceability link back to the original `x_guitar` record.
